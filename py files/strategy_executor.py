class StrategyExecutor():
    ...
    def fab_strategy(self, df, size=7, size2=77, size3=231, size4=721, size5=22):
        ...
        green, orange, black, red, cyan = sma(df, size).values, sma(df, size2).values, sma(df, size3).values, sma(df,
                                                                                                                  size4).values, sma(
            df, size5).values
        ...
        for i in range(231, len(green) - 1):
            ...
            #             ### Rule 1 Higher TF
            #             if cyan[i]>red[i] and black[i]>red[i]:
            #                 if cyan[i]<=black[i]:
            #                     if cyan[i+1]>black[i+1] and trade_history[-1][1] != "Enter" :
            #                         trade_history.append(["Long","Enter",date[i+1], price[i+1], "Rule 1 4H"])

            #             ### Rule 2 Higher TF
            #             if low[i]>black[i] and low[i-1]>black[i-1] and green[i]>black[i] and orange[i]<=black[i] and trade_history[-1][1] != "Enter" :
            #                 if low[i+1] <= black[i+1] and ((orange[i]-orange[i-3])/3)>((black[i]-black[i-3])/3): #If slope of orange MA is greater than black MA
            #                     trade_history.append(["Long","Enter",date[i+1], black[i+1], "Rule 2 4H"])
